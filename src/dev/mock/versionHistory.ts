...
export function restoreVersionHistoryMock(s: MockState, collectionId: string, raw: unknown): VersionHistoryImportResult | undefined {
  ...
  const existing = new Map(s.versions.filter((v) => v.collectionId === collectionId).map((v) => [v.version, v]))
  ...
  for (const e of parsed.entries.filter((x) => x.snapshot).sort((a, b) => compareSemver(a.version, b.version))) {
    ...
    seen.add(e.version)
    let label = e.version
    const clash = existing.get(label)
    if (clash && JSON.stringify(clash.snapshot) === JSON.stringify(e.snapshot) && (clash.notes ?? null) === e.notes) {
      result.skipped++
      result.notes.push(`Version ${e.version} is already in this collection; skipped.`)
      continue
    }
    if (taken.has(label)) {
