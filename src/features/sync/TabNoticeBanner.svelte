...
{#if notice && (notice.kind === 'deleted' || tab.dirty)}
  <div role="status" data-testid="tab-notice" class="flex flex-wrap items-center gap-2 border-b border-warning bg-warning-soft px-3 py-1.5 text-xs text-fg">
    ...
    {#if notice.kind === 'changed'}
      <span class="min-w-0 flex-1">This {tab.example ? 'example' : 'request'} was changed in the cloud while you have unsaved edits. Your edits are untouched.</span>
    ...
    {:else}
      <span class="min-w-0 flex-1">This {tab.example ? 'example' : 'request'} was deleted in the cloud. Your unsaved edits are kept in this tab.</span>
      {#if !sync.blocked && !tab.example}<Button size="sm" onclick={() => saveAsNew(tab)}>Save as new request…</Button>{/if}
