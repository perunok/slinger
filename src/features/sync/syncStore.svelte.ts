...
export class SyncStore {
  ...
  async #flushApplied(): Promise<void> {
    ...
    for (const [tabId, notice] of planNotices(facts(), affected, originals, (id) => app.requestById(id))) {
      ...
      // an example deleted in the cloud has its own banner in the example view (remote === 'gone').
      if (tab && !(tab.example && tab.example.remote !== 'changed' && notice.kind === 'changed')) tab.remoteNotice = notice
    }
  }
