...
**Import** (`services/versionHistory.ts`, called from `importPostmanCollection` inside its transaction; exported as
`restoreVersionHistory(db, collectionId, raw)` for other import paths): the block is untrusted input, validated with zod
