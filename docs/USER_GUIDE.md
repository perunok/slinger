...
- **Top bar:** workspace switcher, "Go to request" (Ctrl+K), environment switcher (with a gear to manage environments), Cloud,
...
Cloud panel traffic also appears here.
...
## Cloud sign-in and publish

The **Cloud** button opens a panel. Set the **API base URL** of your Slinger Cloud server (default `https://api.slinger.app`) and
a **device name**, then start the device sign-in: Slinger shows a code and a button to open the verification page in your browser,
and completes automatically once you approve there (the code expires; you can cancel). Tokens are stored in your OS keychain. When signed in you
see your cloud workspaces, and for the current workspace you can **Publish to cloud** (creates a remote workspace with the same
name and links it), **Link** it to an existing cloud workspace, or **Unlink**. Note that collection sync is not implemented yet:
publishing does not upload your collections. Server setup is documented in the separate `slinger-admin` repository.
